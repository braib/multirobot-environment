#!/usr/bin/env python3
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# Authors: Arshad Mehmood

import os

from ament_index_python.packages import get_package_share_directory
from launch import LaunchDescription
from launch.actions import DeclareLaunchArgument, RegisterEventHandler
from launch.substitutions import LaunchConfiguration
from launch.actions import IncludeLaunchDescription, ExecuteProcess
from launch.launch_description_sources import PythonLaunchDescriptionSource
from launch_ros.actions import Node
from launch.event_handlers import OnProcessExit
from launch.conditions import IfCondition
import launch.logging

def generate_launch_description():
    ld = LaunchDescription()



    use_sim_time = LaunchConfiguration('use_sim_time', default='true')
    declare_use_sim_time = DeclareLaunchArgument(
        name='use_sim_time', default_value=use_sim_time, description='Use simulator time'
    )

    enable_drive = LaunchConfiguration('enable_drive', default='false')
    declare_enable_drive = DeclareLaunchArgument(
        name='enable_drive', default_value=enable_drive, description='Enable robot drive node'
    )

    enable_rviz = LaunchConfiguration('enable_rviz', default='true')
    declare_enable_rviz = DeclareLaunchArgument(
        name='enable_rviz', default_value=enable_rviz, description='Enable rviz launch'
    )

    
    multi_robot = get_package_share_directory('multi_robot')
    
    nav_launch_dir = os.path.join(multi_robot, 'launch', 'nav2_bringup')

    rviz_config_file = LaunchConfiguration('rviz_config_file')

    declare_rviz_config_file_cmd = DeclareLaunchArgument(
        'rviz_config_file',
        default_value=os.path.join(
            multi_robot, 'rviz', 'multi_nav2_default_view.rviz'),
        description='Full path to the RVIZ config file to use')

    burger_urdf = os.path.join(
        multi_robot, 'urdf', 'turtlebot3_burger.urdf'
    )

    waffle_urdf = os.path.join(
        multi_robot, 'urdf', 'turtlebot3_waffle_pi.urdf'
    )


    ld.add_action(declare_use_sim_time)
    ld.add_action(declare_enable_drive)
    ld.add_action(declare_enable_rviz)
    ld.add_action(declare_rviz_config_file_cmd)

    world = os.path.join(
        get_package_share_directory('multi_robot'),
        'worlds', 'turtlebot3_house.world')

    gzserver_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('gazebo_ros'), 'launch', 'gzserver.launch.py')
        ),
        launch_arguments={'world': world}.items(),
    )

    gzclient_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('gazebo_ros'), 'launch', 'gzclient.launch.py')
        ),
    )

    params_file = LaunchConfiguration('nav_params_file')
    declare_params_file_cmd = DeclareLaunchArgument(
        'nav_params_file',
        default_value=os.path.join(multi_robot, 'params', 'nav2_params.yaml'),
        description='Full path to the ROS2 parameters file to use for all launched nodes')
    
     

    ld.add_action(gzserver_cmd)
    ld.add_action(gzclient_cmd)
    ld.add_action(declare_params_file_cmd)

 
    remappings = [('/tf', 'tf'),
                  ('/tf_static', 'tf_static')]
    
    map_server=Node(package='nav2_map_server',
        executable='map_server',
        name='map_server',
        output='screen',
        parameters=[{'yaml_filename': os.path.join(multi_robot, 'map', 'turtlebot3_house.yaml'),
                     },],
        remappings=remappings)

    map_server_lifecyle=Node(package='nav2_lifecycle_manager',
            executable='lifecycle_manager',
            name='lifecycle_manager_map_server',
            output='screen',
            parameters=[{'use_sim_time': use_sim_time},
                        {'autostart': True},
                        {'node_names': ['map_server']}])


    ld.add_action(map_server)
    ld.add_action(map_server_lifecyle)

    ######################

    # Remapping is required for state publisher otherwise /tf and /tf_static 
    # will get be published on root '/' namespace
    # remappings = [('/tf', 'tf'), ('/tf_static', 'tf_static')]

    # last_action = None
    # Spawn turtlebot3 instances in gazebo
    # for robot in robots:

    # namespace = [ '/' + robot['name'] ]

    # Create state publisher node for that instance
    burger_state_publisher = Node(
        package='robot_state_publisher',
        namespace='/burger',
        executable='robot_state_publisher',
        output='screen',
        parameters=[{'use_sim_time': use_sim_time,
                        'publish_frequency': 10.0}],
        remappings=remappings,
        arguments=[burger_urdf],
    )

    # Create spawn call
    spawn_turtlebot3_burger = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
        arguments=[
            '-file', os.path.join(multi_robot,'models', 'turtlebot3_burger', 'model.sdf'),
            '-entity', 'turtlebot3_burger',
            '-robot_namespace', 'burger',
            '-x', '-1.0', '-y', '2.0',
            '-z', '0.01', '-Y', '0.0',
            '-unpause',
        ],
        output='screen',
    )


    waffle_state_publisher = Node(
        package='robot_state_publisher',
        namespace='/waffle',
        executable='robot_state_publisher',
        output='screen',
        parameters=[{'use_sim_time': use_sim_time,
                        'publish_frequency': 10.0}],
        remappings=remappings,
        arguments=[waffle_urdf],
    )

    # Create spawn call
    spawn_turtlebot3_waffle = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
        arguments=[
            '-file', os.path.join(multi_robot,'models', 'turtlebot3_waffle_pi', 'model.sdf'),
            '-entity', 'turtlebot3_waffle',
            '-robot_namespace', 'waffle',
            '-x', '-1.0', '-y', '1.0',
            '-z', '0.01', '-Y', '0.0',
            '-unpause',
        ],
        output='screen',
    )

    ld.add_action(burger_state_publisher)
    ld.add_action(spawn_turtlebot3_burger)
    ld.add_action(waffle_state_publisher)
    ld.add_action(spawn_turtlebot3_waffle)




    burger_bringup_cmd = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(nav_launch_dir, 'bringup_launch.py')),
                launch_arguments={  
                                'slam': 'False',
                                'namespace': '/burger',
                                'use_namespace': 'True',
                                'map': '',
                                'map_server': 'False',
                                'params_file': params_file,
                                'default_bt_xml_filename': os.path.join(
                                    get_package_share_directory('nav2_bt_navigator'),
                                    'behavior_trees', 'navigate_w_replanning_and_recovery.xml'),
                                'autostart': 'true',
                                'use_sim_time': use_sim_time, 'log_level': 'warn'}.items()
                                )

    waffle_bringup_cmd = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(nav_launch_dir, 'bringup_launch.py')),
                launch_arguments={  
                                'slam': 'False',
                                'namespace': '/waffle',
                                'use_namespace': 'True',
                                'map': '',
                                'map_server': 'False',
                                'params_file': params_file,
                                'default_bt_xml_filename': os.path.join(
                                    get_package_share_directory('nav2_bt_navigator'),
                                    'behavior_trees', 'navigate_w_replanning_and_recovery.xml'),
                                'autostart': 'true',
                                'use_sim_time': use_sim_time, 'log_level': 'warn'}.items()
                                )

    ld.add_action(burger_bringup_cmd)
    ld.add_action(waffle_bringup_cmd)






    # Save last instance for next RegisterEventHandler
    # last_action = spawn_turtlebot3_burger
    ######################

    ######################
    # Start rviz nodes and drive nodes after the last robot is spawned



    # Create a initial pose topic publish call
    # burger_message = '{header: {frame_id: map}, pose: {pose: {position: {x: ' + \
    #     '0.0' + ', y: ' + '0.02' + \
    #     ', z: 0.01}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0000000}}, }}'

    burger_message = '{header: {frame_id: map}, pose: {pose: {position: {x: -1.0, y: 2.0, z: 0.01}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0000000}}}}'


    # burger_initial_pose_cmd = ExecuteProcess(
    #     cmd=['ros2', 'topic', 'pub', '-t', '3', '--qos-reliability', 'reliable', 'burger' + ['/initialpose'],
    #         'geometry_msgs/PoseWithCovarianceStamped', burger_message],
    #     output='screen'
    # )

    burger_initial_pose_cmd = ExecuteProcess(
        cmd=['ros2', 'topic', 'pub', '-t', '3', '--qos-reliability', 'reliable', '/burger/initialpose',
            'geometry_msgs/PoseWithCovarianceStamped', burger_message],
        output='screen'
    )

    burger_rviz_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(nav_launch_dir, 'rviz_launch.py')),
            launch_arguments={'use_sim_time': use_sim_time, 
                                'namespace': '/burger',
                                'use_namespace': 'True',
                                'rviz_config': rviz_config_file, 'log_level': 'warn'}.items(),
                                condition=IfCondition(enable_rviz)
                                )

    drive_turtlebot3_burger = Node(
        package='turtlebot3_gazebo', executable='turtlebot3_drive',
        namespace='/burger',
        output='screen',
        condition=IfCondition(enable_drive),
    )

    ld.add_action(burger_initial_pose_cmd)
    ld.add_action(burger_rviz_cmd)
    ld.add_action(drive_turtlebot3_burger)





    # Create a initial pose topic publish call
    # waffle_message = '{header: {frame_id: map}, pose: {pose: {position: {x: ' + \
    #     '0.0' + ', y: ' + '0.02' + \
    #     ', z: 0.01}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0000000}}, }}'

    waffle_message = '{header: {frame_id: map}, pose: {pose: {position: {x: -1.0, y: 1.0, z: 0.01}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0000000}}}}'

    # waffle_initial_pose_cmd = ExecuteProcess(
    #     cmd=['ros2', 'topic', 'pub', '-t', '3', '--qos-reliability', 'reliable', 'waffle' + ['/initialpose'],
    #         'geometry_msgs/PoseWithCovarianceStamped', waffle_message],
    #     output='screen'
    # )

    waffle_initial_pose_cmd = ExecuteProcess(
        cmd=['ros2', 'topic', 'pub', '-t', '3', '--qos-reliability', 'reliable', '/waffle/initialpose',
            'geometry_msgs/PoseWithCovarianceStamped', waffle_message],
        output='screen'
    )

    waffle_rviz_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(nav_launch_dir, 'rviz_launch.py')),
            launch_arguments={'use_sim_time': use_sim_time, 
                                'namespace': '/waffle',
                                'use_namespace': 'True',
                                'rviz_config': rviz_config_file, 'log_level': 'warn'}.items(),
                                condition=IfCondition(enable_rviz)
                                )

    drive_turtlebot3_waffle = Node(
        package='turtlebot3_gazebo', executable='turtlebot3_drive',
        namespace='/waffle',
        output='screen',
        condition=IfCondition(enable_drive),
    )

    ld.add_action(waffle_initial_pose_cmd)
    ld.add_action(waffle_rviz_cmd)
    ld.add_action(drive_turtlebot3_waffle)


    return ld
